return {
    ...
    version = '*',
    cmd = { 'ToggleTerm', 'TerminalBottom', 'CompileCpp' },
    keys = { '<F6>', '<F5>' },
    config = function()
        ...
        local function run_cpp()
            ...
            local cmd = string.format('cd %s && g++ -std=c++17 %s -o %s && %s', file_dir, file, output_path, output_path)
